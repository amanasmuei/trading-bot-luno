...
def check_docker_compose():
    ...
    try:
        ...
        if result.returncode == 0:
            print(
                f"{Colors.GREEN}✅ Docker Compose found: {result.stdout.strip()}{Colors.END}"
            )
            return True
        else:
            ...
            if result.returncode == 0:
                print(
                    f"{Colors.GREEN}✅ Docker Compose (legacy) found: {result.stdout.strip()}{Colors.END}"
                )
                return True
            else:
...
def check_env_file():
    ...
    if env_file.exists():
        ...
        print(
            f"{Colors.CYAN}💡 Run setup wizard first: python setup_wizard.py{Colors.END}"
        )
        return False
...
def build_image():
    ...
    try:
        result = subprocess.run(
            ["docker", "build", "-t", "luno-trading-bot", "."], check=True
        )
...
def run_container(dry_run=True):
    ...
    try:
        ...
        print(f"{Colors.CYAN}Dashboard: http://localhost:5001{Colors.END}")
...
def main():
    ...
    while True:
        ...
        try:
            choice = input(
                f"{Colors.WHITE}Enter your choice (1-8): {Colors.END}"
            ).strip()
            ...
            if choice == "1":
                ...
                if not has_config:
                    print(
                        f"{Colors.RED}❌ Please set up configuration first (option 7){Colors.END}"
                    )
                else:
                ...
                if not has_config:
                    ...
                    print(
                        f"{Colors.RED}⚠️  WARNING: This will start live trading!{Colors.END}"
                    )
                ...
                print(
                    f"{Colors.YELLOW}⚠️  Invalid choice. Please enter 1-8.{Colors.END}"
                )
